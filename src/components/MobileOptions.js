...
import MobileOtherOptions from './MobileOtherOptions';
...
const MobileOptions = () => {
  ...
  return (
    <div className=' md:p-2 block md:hidden'>
        {menu &&
          <MobileOtherOptions/>
        }
        
        {showMessageFour&& <img  className='hover:cursor-pointer w-60 h-20   fixed  bottom-8 right-1 p-6 block md:hidden  ' src="../Hover message (4).png" alt='button'/>}
        {showMessageThree&& <img className='hover:cursor-pointer w-60 h-20   fixed bottom-8 right-1 p-6 block md:hidden ' src="../Hover message (2).png" alt='button'/>}
        {showMessageTwo&& <img  className='hover:cursor-pointer w-60 h-20   fixed bottom-8 right-1 p-6 block md:hidden ' src="../Hover message (3).png" alt='button'/>}
        {showMessageOne&& <img  className='hover:cursor-pointer w-60 h-20  fixed bottom-8 right-1 p-6 block md:hidden ' src="../Hover message (1).png" alt='button'/>}
      
        {showButton&& <img onClick={()=>popUpConrtol()} className='hover:cursor-pointer w-20 h-20   fixed bottom-0 right-0 p-4 block md:hidden ' src="../feedback.png" alt='button'/>}
        {showFeedback&& 
        <>
        <Option/>
        <ShareFeedback/>
